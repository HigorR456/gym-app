...
- `scheduled_programs`: needs a status field (e.g. `active` / `archived`), the current cycle position, the starting day chosen at creation, and a nullable end date (for continuous/infinite programs — see [Schedule](../features/schedule.md)). This status field is that entity's own soft-delete/soft-cancel mechanism — it doesn't also need a generic `deletedAt`. Also needs a `pending_since_date` (`TEXT`, local calendar date): the date the day at `current_cycle_position` became pending — found while building the scheduling engine (step 10), required to know how overdue a stuck workout day is, since a fixed `start_date + N*position` formula can't account for the cumulative slide (see [Schedule](../features/schedule.md), "Automatic reschedule on missed checkout"). Added to the schema once the Schedule repository is built (step 11); the domain engine (`src/domain/schedule/`) treats it as part of `ScheduleState` regardless.
...
- Foreign key policy (enforced via `PRAGMA foreign_keys = ON`): child rows that only exist as part of their parent (e.g. `workout_exercises`/`workout_sets` under a `workout`) use `ON DELETE CASCADE`. Cross-entity references that the spec requires explicit app-level handling for before the referenced row can go — deleting a `Workout` referenced by `program_days` (see [Workout](../features/workout.md)) — use no action, so a raw delete that skips the required app logic fails loudly instead of silently corrupting state. `workout_sessions`' links back to the workout/program/schedule it came from use `ON DELETE SET NULL`, since history must outlive them — `scheduled_programs.program_id` uses the same `ON DELETE SET NULL` for the same reason (added in `0003_schedule_engine.ts`, step 11): a Program that was ever scheduled must stay deletable (after its active schedule is archived, per [Program](../features/program.md), "Editing/deleting a Program with an active schedule") without losing that now-orphaned schedule's history.
- `exercises` columns match the real RepDB dataset schema (see [Dataset & Licensing](dataset-and-licensing.md), "Confirmed dataset schema"): `id`, `name_en`/`name_es`/`name_de`, `description_en`/`description_es`/`description_de`, `body_part`, `equipment`, `difficulty`, `is_bodyweight` are real columns; everything else the dataset provides (muscle breakdown, instructions, tips, category, tags, ...) lives in a `metadata` JSON column, and image references live in an `images` JSON column (keys into the generated image map — see [Dataset & Licensing](dataset-and-licensing.md)). Promote a `metadata`/`images` field to a real column only once a feature needs to query on it, not preemptively.
- The catalog is seeded (not migrated) into `exercises` on first run — `src/data/sqlite/bootstrapDatabase.ts` runs schema migrations, then catalog seeding, as two separate, independently-idempotent steps (a dataset update shouldn't require a fake schema migration bump, and a schema migration shouldn't have to know about seed data).
